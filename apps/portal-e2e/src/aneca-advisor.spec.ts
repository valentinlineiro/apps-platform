...
test.describe('aneca-advisor', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('https://localhost/aneca-advisor', { waitUntil: 'networkidle', timeout: 20000 });
  ...
  test('fields load from API (no 401)', async ({ page }) => {
    // Field selector is populated once /api/aneca/fields returns data
  ...
  test('evaluate button is present on the Idoneidad tab', async ({ page }) => {
    await page.locator('aneca-advisor-app button.tab-btn', { hasText: 'Idoneidad' }).click();
    await expect(
      page.locator('aneca-advisor-app button.btn-primary', { hasText: 'Evaluar ahora' })
    ).toBeVisible();
  });
